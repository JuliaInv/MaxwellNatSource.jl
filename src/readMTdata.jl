...
export readMTdata, readZTEMdata
...
function readZTEMdata(filename::String,
                      trx::Array{TransmitterOmega})
   
   ntrx = length(trx)   
   fieldsPerData = 3

   ncomp = 2*2  #  # of components
   ignore = -99
   ...
   for itx = 1:ntrx
      ...
      zdata = Array{Complex128}(2,nr)
      zWd   = Array{Complex128}(2,nr)
      ...
      for ir = 1:nr
         ...
         zdata[2,ir] = complex( dobs[1], dobs[2] )
         zdata[1,ir] = complex( dobs[3], dobs[4] )
         
         zWd[2,ir] = complex( sd[1], sd[2] )
         zWd[1,ir] = complex( sd[3], sd[4] )
